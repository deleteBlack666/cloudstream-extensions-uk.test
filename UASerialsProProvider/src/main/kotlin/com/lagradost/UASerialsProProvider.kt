        ...
        return if (tortugaSeasons != null) {
            ...
            tortugaSeasons.forEachIndexed { seasonsIndex, season ->
                ...
                season.folder.forEachIndexed { episodesIndex, episode ->
                    ...
                    val episodeThumb = try {
                        val decodedUrl = if (episode.file.startsWith("http")) episode.file
                                         else Decoder.tortugaDecode(episode.file) ?: ""
                        // формат після декодування: {Назва}https://...
                        // або просто https://...
                        val cleanUrl = if (decodedUrl.contains("}")) decodedUrl.substringAfter("}") else decodedUrl
                        val slug = cleanUrl
                            .substringAfter("/hls/serials/", "")
                            .substringBefore("/hls/")
                        if (slug.isNotBlank())
                            "https://calypso.tortuga.tw/content/stream/serials/$slug/screen.jpg"
                        else null
